...
const int max_allowable_line_number = 200000;
...
int main(){
    ...
    dest_file_amount =dest_file_amount*2;

while (testcase--){
/********取得样本txt的文件名**********/
    cout<<"即将生成"<<dest_file_amount<<"个文件"<<endl;
...
/********从所有样本txt中取出代码行**********/ 
    ...
    while(tot_line_number<max_allowable_line_number){
        ...
        while (tot_line_number<max_allowable_line_number  && getline(fin, line[tot_line_number++])){
            rep(i, 0, 4){
                if (rand()%2){
                    getline(fin, line[tot_line_number-1]);
                }
