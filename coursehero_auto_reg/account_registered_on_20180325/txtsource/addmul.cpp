...
ini[2*node+1]=multiply(ini[node],(b-mid));
}
...
if(a >= i && b <= j)
{
tree[node]=add(tree[node],multiply(((b-a+1)),(value)));
//cout<<"tree "<< node<<" is now"<<tree[node]<<endl;
if(a != b) {
adds[2*node]=add(adds[2*node],multiply(((mid-a+1)),(value)));
//cout<<"adds "<<2*node<<" is now"<<adds[2*node]<<endl;
adds[2*node+1]=add(adds[2*node+1],multiply(((b-mid)),(value)));
...
ini[2*node+1]=multiply(((b-mid)),(value));
adds[2*node]=adds[2*node+1]=0;
mul[2*node]=mul[2*node+1]=1;
...
update_tree3(1+node*2, 1+(a+b)/2, b, i, j, value);
tree[node] = add(tree[node*2],tree[node*2+1]);
}
...
{
scanf("%lld",&v);
update_tree3(1,0,n-1,a-1,b-1,v);
}
else if(dec==4)
{
